        ...
        <nav className={`container navbar navbar-expand-md px-4 px-md-0 ${borderBottom && 'borderBottom'} ${isLocation ? 'navbar-dark' : 'navbar-light'}`}>
            ...
            <div className="collapse navbar-collapse text-center" id="navbarSupportedContent">
                ...
                <ul className="navbar-nav ml-auto">
                    ...
                    <li className="nav-item">
                        {
                            sessionStorage.getItem('name') ? <button title="Click to LogOut" className="btn" onClick={signOut}>{sessionStorage.getItem('name')}</button> : <Link className="btn" to="/login">LogIn</Link>
                        }
                    </li>
                </ul>
